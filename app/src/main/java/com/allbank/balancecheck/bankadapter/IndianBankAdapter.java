...
public class IndianBankAdapter extends RecyclerView.Adapter<IndianBankAdapter.ViewHolder> {

    Context context;
    ArrayList<BankModel> IndianBanks;

    public IndianBankAdapter(Context context, ArrayList<BankModel> IndianBanks){
        this.context = context;
        this.IndianBanks = IndianBanks;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.imageView.setImageResource(IndianBanks.get(position).img);
        holder.bank_name.setText(IndianBanks.get(position).bankname);
    }

    @Override
    public int getItemCount() {
        return IndianBanks.size();
    }
