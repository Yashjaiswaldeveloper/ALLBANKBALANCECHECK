...
public class JapanBankAdapter extends RecyclerView.Adapter<JapanBankAdapter.ViewHolder> {

    Context context;
    ArrayList<BankModel> JapanBanks;

    public JapanBankAdapter(Context context, ArrayList<BankModel> JapanBanks){
        this.context = context;
        this.JapanBanks = JapanBanks;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.imageView.setImageResource(JapanBanks.get(position).img);
        holder.bank_name.setText(JapanBanks.get(position).bankname);
    }

    @Override
    public int getItemCount() {
        return JapanBanks.size();
    }
